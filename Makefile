...
build: ## Build the container
	docker build --platform $(PLATFORM) --build-arg="${MASTODON_VERSION}" --build-arg="${DOCKER_GUID}" -t $(APP_NAME) .

build-nc: ## Build the container without caching
	docker build --platform $(PLATFORM) --build-arg="${MASTODON_VERSION}" --build-arg="${DOCKER_GUID}" --no-cache -t $(APP_NAME) .
